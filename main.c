...
static void node_init(struct clock *clock, struct node *parent, struct node *node) {
	node->clock = clock;
	node->parent = parent;
...
#define NODES 16
	...
	for (int i = 0; i < NODES; ++i) {
		if (pthread_create(&threads[i], NULL, thread_fn, &nodes[i]) != 0) {
			return EXIT_FAILURE;
		}
	}

	clock_tick(&clock);
	clock_wait(&clock, 1 << NODES);
	clock_stop(&clock);
